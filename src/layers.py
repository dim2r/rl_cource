...
class MultinomialLayer(torch.nn.Module):
    '''
    ...
    def __call__(self, log_action_probs):
        return torch.distributions.categorical.Categorical(logits=log_action_probs)
    
    
class DiagGaussianLayer(torch.nn.Module):
    '''
    ...
    def __init__(self, output_dim=None, log_std=None):
        ...
        if log_std is None:
            self.log_std = torch.nn.Parameter(torch.zeros(output_dim), requires_grad=True)

    def __call__(self, mean):
        std = torch.exp(self.log_std)
        normal_dist =  torch.distributions.Independent(torch.distributions.normal.Normal(loc=mean, scale=std), 1)
...
def build_mlp(input_dim, hidden_dims, output_dim):
    '''
    Build a multilayer perceptron(mlp) with tanh activations with the specified input,
    ...
    mlp = torch.nn.Sequential(*build_layers(input_dim, hidden_dims, output_dim))
...
def build_diag_gauss_policy(state_dim, hidden_dims, action_dim,
    ...
    policy = torch.nn.Sequential(*layers)
